...
function showScreen(id) {
  ...
  if (!target) {
    const route = window.guidePageMap?.[id];
    if (route) {
...
function switchTab(name, el) {
  ...
  if (!panel) {
    const pageMap = window.guidePageMap || {
...
function confirmBooking(btn) {
  ...
  btn.closest(".booking-actions").innerHTML =
...
function updateBadge() {
  ...
  if (pending === 0)
    document.getElementById("booking-badge").style.display = "none";
}
